...
export default function ServiceStatusesPage({ initialRows }: { initialRows: ServiceStatus[] }) {
  const { hasPermission } = useAuth()
  const canEdit = hasPermission('settings.manage')
  const [rows, setRows] = useState<ServiceStatus[]>(initialRows)
  ...
  const load = () => getServiceStatuses().then(setRows)
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setError(null)
    const res = editing
      ? await updateServiceStatus(editing.id, data.label, data.color)
      : await createServiceStatus(data.label, data.color)
    setSaving(false)
    if (res.ok === false) {
      setError(res.error)
    } else {
  ...
  const toggleActive = async (s: ServiceStatus) => {
    await toggleServiceStatus(s.id, !s.is_active)
    load()
  }
  ...
  const move = async (index: number, dir: -1 | 1) => {
    ...
    if (!current) return
    await moveServiceStatus(current.id, dir)
    load()
  }
    ...
    <div className="w-full max-w-4xl space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-xl font-bold text-foreground sm:text-2xl">Service Statuses</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Lab-to-doctor instruction printed on delivery orders.</p>
          </div>
        </div>
        {canEdit && <Button className="w-full sm:w-auto" onClick={openNew}><Plus className="h-4 w-4 mr-2" />Add Status</Button>}
      </div>
                  ...
                  <TableCell>
                    {canEdit && (
                      <div className="flex items-center gap-2">
                        <TableActionButton label="Edit service status" icon={PencilLine} tone="primary" onClick={() => openEdit(s)} />
                        <ActiveSwitch
                          checked={s.is_active}
